...
export const actions = {
    ...
    async get_all({ commit }: any, { query }: any) {
        ...
        const response = await Api().get('/pinned', {
            params: query
        })
    ...
    async search_genre({ commit }: any, { name }: any) {
        ...
        const response = await Api().get(`/pinned?name=${name}`)
        // console.log('all file', response.data);
        return response.data
    },
    async get_one({ commit }: any, { id }: any) {
        const response =  await Api().get(`pinned/${id}`)
        return response.data
    ...
    async add_item({ commit }: any, { valueForm  }: any){
        ...
        const response = await Api().post('/pinned', valueForm )
        .then((res: any) => {
    ...
    async update_genre({ commit }: any, { id, updateForm }: any){
        const response = await Api().put(`/pinned/${id}`, updateForm )
        .then((res: any) => {
            console.log('response update', res.data);
    ...
    async delete({ commit }: any, { id }: any){
        const response = await Api().delete(`/pinned/${id}`)
