...
export const actions = {
    ...
    async search_artist({ commit }: any, { name }: any) {

        const response = await Api().get(`/artist?name=${name}`)
        // console.log('all file', response.data);
    ...
    async add_item({ commit }: any, { valueForm  }: any){
        ...
        const response = await Api().post('/artist', valueForm )
        .then((res: any) => {
    ...
    async update_artist({ commit }: any, { id, updateForm }: any){
        const response = await Api().put(`/artist/${id}`, updateForm )
        .then((res: any) => {
            console.log('response update', res.data);
